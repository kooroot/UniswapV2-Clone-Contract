...
contract UniswapV2Router {
    ...
    function removeLiquidity(
        ...
        console.log("Removing liquidity from pair:", pair);
        console.log("LP token amount:", liquidity);
        
        // LP 토큰을 페어 컨트랙트로 전송
        IERC20(pair).safeTransferFrom(msg.sender, pair, liquidity);
        console.log("LP tokens transferred to pair");
        
        // 페어 컨트랙트에서 토큰을 소각하고 토큰을 반환
        (uint amount0, uint amount1) = IUniswapV2Pair(pair).burn(to);
        console.log("Burned LP tokens, received amounts - amount0:", amount0, "amount1:", amount1);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to;
            if (i < path.length - 2) {
                address pairAddress1 = IUniswapV2Factory(factory).getPair(output, path[i + 2]);
                if (pairAddress1 == address(0)) {
                    pairAddress1 = IUniswapV2Factory(factory).createPair(output, path[i + 2]);
                }
                to = pairAddress1;
            } else {
                to = _to;
            ...
            // 현재 pair 주소 가져오기
            address pairAddress2 = IUniswapV2Factory(factory).getPair(input, output);
            require(pairAddress2 != address(0), "UniswapV2Router: PAIR_NOT_FOUND");
            
            IUniswapV2Pair(pairAddress2).swap(
    ...
    function swapExactTokensForTokens(
        ...
        IERC20(path[0]).safeTransferFrom(msg.sender, pairAddress, amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(pairAddress, amounts[0]));
        _swap(amounts, path, to);
    }
